...
-testerl_export( { [], sc_rank_testsuite } ).  % todo needs test suite
...
%% @spec simple(Values::numericlist()) -> rankinglist()
...
%% @spec tied(Values::numericlist()) -> rankinglist()
...
tied(List) ->
    ...
    tied_rank_worker(simple(List), [], no_prev_value).
...
%% @spec ordered(Values::numericlist()) -> rankinglist()
...
ordered([Front|Rem], Ranks, Work) ->

    {value,Item}  = lists:keysearch(Front,2,Ranks),
    {IRank,Front} = Item,
    ordered(Rem, Ranks--[Item], [{IRank,Front}]++Work).
